function u_bar = get_u_bar( x, h, dS_dx, E, fs, A_T )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
elseif (deformation_plus_sliding == 1)
    ...
    deformation_term = ( ( (2*E.*A_T)./(n+2) ) .* ...
               (rho_ice * g * abs(dS_dx) ).^n ) .* h.^(n+1);
    
    sliding_term = ( (fs .* (rho_ice * g * abs(dS_dx) ).^n ) .* h.^(n-1) );
    
    u_bar = - sign(dS_dx) .* (deformation_term + sliding_term);
