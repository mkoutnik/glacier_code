% run_min_search_width


width_vec     = [ 0.5:0.5:10 ];
N_width       = length( width_vec );

length_run    = length(x_P);
save_width    = [];
save_mismatch = NaN * ones(length_run, N_width);
...
RMS_mismatch = NaN * ones( 1, N_width );


%  Loop over width
 for i_width = 1:N_width

      width_use = width_vec(i_width);
...
% ==============
 ...
 % Surface velocity:
   ...
   best_width = width_vec(index)
   save_width = [ save_width best_width ];    
